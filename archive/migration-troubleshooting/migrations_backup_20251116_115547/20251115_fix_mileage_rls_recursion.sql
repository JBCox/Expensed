...
COMMENT ON POLICY "Finance read trips" ON mileage_trips IS
  'Finance and admin users can read all mileage trips. Uses auth.user_role() helper to avoid RLS recursion.';

COMMENT ON POLICY "Finance read segments" ON mileage_segments IS
  'Finance and admin users can read all mileage segments. Uses auth.user_role() helper to avoid RLS recursion.';
